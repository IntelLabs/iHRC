...
//#ifdef SVM
// BTL 20101117 For the Windows SVM prototype, we allocate out of a single 128MB shared region.
void *svmRegion;

// BTL 20101129 The next available and limit pointers for the simple bump-pointer svmRegion allocator. 
void *nextSVMAvailable;
void *svmRegionLimit;

// BTL 20101118 The allocation granularity for getRawMemory/rawAlloc.
...
static void initMemoryManager()
{
	...
	//#ifdef SVM
    // BTL 20101118 Find the allocation granularity for rawAlloc.
    SYSTEM_INFO sysInfo;
    GetSystemInfo(&sysInfo);
    allocationGranularity = sysInfo.dwPageSize; 
    TRACEF(( "[ScalableMalloc trace] SVM allocationGranularity = %u\n", allocationGranularity ));

    // Create the 128MB shared region that holds shared data.
    svmRegion = malloc( SVM_SIZE * 2 + allocationGranularity);
	if (!svmRegion) {
        fprintf (stderr, "Failed to allocate memory for SVM region.\n");
        exit(-1);
    }
	svmRegion = roundToNextRawAllocBoundary(svmRegion);
	/* Make sure SVM region does not cross 4GB boundaries, so that our compiler can use lower 32 bits of a pointer
	   to calculate the offset in a GEN address
	*/
	if ((uint32_t)((uint64_t)svmRegion + SVM_SIZE) < (uint32_t)(uint64_t)svmRegion) {
		svmRegion = (char*)svmRegion + SVM_SIZE;
	}
    // svmRegion is now aligned to the system's allocation-granularity (e.g., a 4K page boundary).
    nextSVMAvailable = svmRegion;
    svmRegionLimit   = (void *)( (uintptr_t)svmRegion + SVM_SIZE );
	// RB Initializing SVM
	for (uintptr_t ptr = (uintptr_t)svmRegion; ptr < (uintptr_t)svmRegionLimit; ptr++) 
		*(unsigned int *)ptr = 0;

	//fprintf (stdout, "allocate shared memory for SVM region.\n");
    TRACEF(( "[ScalableMalloc trace] VirtualAlloc'ed a %d MB shared region from [0x%p..0x%p)\n",
             (SVM_SIZE >> 20), svmRegion, svmRegionLimit ));
    TRACEF(( "[ScalableMalloc trace] nextSVMAvailable = 0x%p\n",
             nextSVMAvailable ));
//#endif // SVM
...
//#ifdef SVM
// BTL 20101117 Implementation of the public function that returns a pointer 
// to the start of the 128MB SVM region used to hold shared data.
extern "C" void * get_svm_region()
{
    if (!isMallocInitialized()) 
        doInitialization();

    TRACEF(( "[ScalableMalloc trace] get_svm_region called, returning SVM region 0x%X\n", svmRegion ));
	return svmRegion;
}
//#endif // SVM
