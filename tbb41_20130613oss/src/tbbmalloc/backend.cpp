...
//#ifdef SVM
// BTL 20101117 Allocate raw "OS" memory (pages) out of a single 128MB shared region.

// Rounds "p" to the next "alignment" boundary. Assumes "alignment" is a power of 2 like 4K.
#define ROUND_UP(p, alignment) \
	((void *)( (((uintptr_t)(p)) + (alignment)-1) & (~((alignment)-1)) ))

// Returns the pointer "p" rounded up to the next allocationGranularity boundary. 
void *roundToNextRawAllocBoundary(void *p) {
    void *ret = ROUND_UP(p, allocationGranularity);
    return ret;
}

void* getRawMemory (size_t size, bool = false) {
    void *ret = roundToNextRawAllocBoundary(nextSVMAvailable);
    TRACEF(( "[ScalableMalloc trace] getRawMemory: ret = 0x%p, regionLimit = 0x%p\n", 
             ret, svmRegionLimit ));
    if (((uintptr_t)ret) > (uintptr_t)svmRegionLimit) {
        return NULL;
    }
    nextSVMAvailable = (void *)((uintptr_t)nextSVMAvailable + size);
    return ret;
}
//#else  // !SVM
...
//#ifdef SVM
    // BTL 20101117 We never actually free raw "OS" memory out of the 128MB shared region
    // but instead try hard to reuse it.
    return TRUE;
//#else  // !SVM
